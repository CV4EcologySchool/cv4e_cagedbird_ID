...
# Parameters
config = '/home/home01/bssbf/cv4e_cagedbird_ID/all_model_states/ep100_57sp_ahorflip0.5_lr1e-2_snone_orig/config_ep100_57sp_ahorflip0.5_lr1e-2_snone_orig.yaml'
...
class_mapping_file = '/home/home01/bssbf/cv4e_cagedbird_ID/ct_classifier/class_mapping_57.pickle'
with open(class_mapping_file, 'rb') as f:
    class_mapping = pickle.load(f)
print(class_mapping)

# Output directories for annotated and raw images
output_dir_match = 'predicted_images/match'
output_dir_mismatch = 'predicted_images/mismatch'
output_dir_match_raw = 'predicted_images/match_raw'
output_dir_mismatch_raw = 'predicted_images/mismatch_raw'
...
with open('validation_predictions14.csv', mode='w', newline='') as file:
...
class_metrics_df.to_csv("species_class_metrics_unknown.csv", index=False)
...
plt.tight_layout()
plt.savefig("confusion_matrix_normalized_unknown.png")
plt.show()
...
for species in low_f1_species:
    ...
    plt.tight_layout()
    plt.savefig(f"histogram_unknown{species}.png")
    plt.show()
...
plt.grid()
plt.savefig("precision_recall_curve_micro_unknown.png")
plt.show()
